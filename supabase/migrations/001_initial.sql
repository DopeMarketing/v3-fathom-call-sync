...
CREATE TABLE sync_configurations (
  ...
  duplicate_handling TEXT NOT NULL CHECK (duplicate_handling IN ('skip', 'overwrite', 'rename')) DEFAULT 'skip',
  auto_sync_enabled BOOLEAN NOT NULL DEFAULT false,
  sync_frequency_hours INTEGER,
  file_naming_pattern TEXT NOT NULL DEFAULT '{title}_{date}',
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE TABLE sync_job_items (
  ...
  status TEXT NOT NULL CHECK (status IN ('pending', 'success', 'failed', 'skipped')) DEFAULT 'pending',
...
CREATE UNIQUE INDEX idx_uploaded_files_user_gdrive_file ON uploaded_files(user_id, google_drive_file_id);
